...
lineplot_genes_list <- list(list_1 = c("E2f1","E2f2","Myc"),
                       ...
                       list_4_2 = c("Ccl21a",
                                  "Aire","Fezf2"),
                       list_5 = c("Cat","Cox7b","Rtn3","Gpx4","Mdh2"),
                       list_6 = c("Mcm2","Mcm3","Mcm4","Mcm5","Mcm6"))

data_for_lineplot <- target_demoData_Epithelial[,ROIs]

assayDataElement(object = target_demoData_Epithelial, elt = "log_q") <-
  assayDataApply(target_demoData_Epithelial, 2, FUN = log, base = 2, elt = "q_norm")

data_for_boxplot <- target_demoData_Epithelial

library(ggplotify)

for (list in names(lineplot_genes_list)){
  for (gene in lineplot_genes_list[[list]]){
    
  gene_expr <- data.frame(sample = names(data_for_boxplot@assayData$log_q[gene,]), gene = data_for_boxplot@assayData$log_q[gene,])
  
  gene_expr$origin <- pData(data_for_boxplot)$SampleCellType[match(gene_expr$sample,rownames(pData(data_for_boxplot)))]
  
  write.csv(gene_expr, paste(gene,"_boxplot_data",".csv",sep = ""))
  
  res_aov <- aov(gene_expr$gene ~ gene_expr$origin)
  
  residual_df <- data.frame(index = names(res_aov$residuals), value = res_aov$residuals)
  # Check for normality 
  histogram <- ggplot(residual_df, aes(x = index, y = value)) + geom_histogram()
  
  #ggsave(histogram, filename = paste(gene,"hist.png", sep = "_"))
  
  library(car)
  qqPlot(res_aov$residuals,
  id = FALSE)
  
  rownames(gene_expr) <- NULL
  colnames(gene_expr)[2] <- gene
  
  g_box <- ggplot(gene_expr, aes(x = origin, y = gene_expr[,2])) + geom_boxplot() + ylab(gene)
  
  boxplot_name <- paste(gene,"box",sep = "_")
  
  ggsave(filename = paste(boxplot_name,".png",sep = ""), plot = g_box)
}}

for (list in names(lineplot_genes_list)){
  genes <- lineplot_genes_list[[list]]
  
  data_for_lineplot_sub <- data_for_lineplot[genes,]
  
  df_line <- as.data.frame(t(data_for_lineplot_sub@assayData$q_norm))
  
  df_line$sample <- rownames(df_line)
  
  df_line_processed <- pivot_longer(data = df_line, !sample, names_to = "gene", values_to = "scaled counts (Q3 norm)")
  
  df_line_processed$cluster <- pData(data_for_lineplot_sub)$kmeans_cluster[match(df_line_processed$sample,rownames(pData(data_for_lineplot_sub)))]
  
  df_line_processed$sample <- gsub(".*-","",df_line_processed$sample)
  
  df_line_processed$sample <- gsub(".dcc","",df_line_processed$sample)
  
  #df_line_processed$cluster_sample <- paste("Cluster",df_line_processed$cluster,df_line_processed$sample, sep = "_")
  
  df_line_processed$sample <- factor(df_line_processed$sample, levels = unique(df_line_processed$sample))
  
  line_plot_test <- ggplot(df_line_processed, aes(x=sample, y = `scaled counts (Q3 norm)`)) + theme_bw() +
  geom_line(aes(color = gene, group = gene),size=1.5) + theme(panel.spacing = unit(0, "cm")) +
  scale_color_manual(values = c("darkred", "steelblue","grey","green","violet","orange")) + theme(axis.text.x = element_text(angle = 90, size = 19), axis.title.x = element_text(size = 20,face = "bold"), axis.title.y = element_text(size = 20, face = "bold"), axis.text.y = element_text(size = 19), legend.text = element_text(size = 19), legend.title = element_text(size = 20,face = "bold"))
  
  print(line_plot_test)
  
}

# save a clean target_demodata for troubleshooting
clean_target_demoData <- target_demoData

samples <- unlist(unique(pData(clean_target_demoData)[1]))
names(samples) <- NULL

sub_clean_target_demoData <- clean_target_demoData

unique(pData(sub_clean_target_demoData)$SampleCellType)

# gsub to get rid of spaces
pData(sub_clean_target_demoData)$SampleCellType <- gsub(" ","_",pData(sub_clean_target_demoData)$SampleCellType)

# remove mcherry
target_demoData_for_DEG <- sub_clean_target_demoData[!grepl("mCherry",rownames(sub_clean_target_demoData))]
```
...
# Start of analysis
comparisons <- c("Thymoma_Epithelial|Thymus_Medullar_Epithelial", 
                 "Thymoma_Epithelial|Thymus_Cortical_Epithelial",
                 "Thymoma_Tcells|Thymus_Medullar_Tcells",
                 "Thymoma_Tcells|Thymus_Cortical_Tcells",
                 "Thymoma_Bcells|Thymus_Bcells")

## For running in reverse order:
comparisons <- c("Thymus_Medullar_Epithelial|Thymoma_Epithelial",
                 "Thymus_Cortical_Epithelial|Thymoma_Epithelial",
                 "Thymus_Medullar_Tcells|Thymoma_Tcells",
                 "Thymus_Cortical_Tcells|Thymoma_Tcells",
                 "Thymus_Bcells|Thymoma_Bcells")

for (comparison in comparisons) {
  
# Filter original data by comparisons to look for  
filtered_data <- pData(target_demoData_for_DEG)[grepl(comparison, pData(target_demoData_for_DEG)$SampleCellType),]

# Check cells are present
print(unique(filtered_data$SampleCellType))
  
fsamplenames <- rownames(filtered_data)

starget_demoData <- target_demoData_for_DEG[,fsamplenames]

# Retrieve counts matrix for GSEA
assayDataElement(object = starget_demoData, elt = "log_q") <-
  assayDataApply(starget_demoData, 2, FUN = log, base = 2, elt = "q_norm")

## Retrieve counts and annotation data for GSEA on NIDAP -----------------------
# Get rid of ".dcc" so NIDAP can recognize sample names
# counts_mtx_GSEA <- starget_demoData@assayData$log_q
# colnames(counts_mtx_GSEA) <- gsub("\\..*","",colnames(counts_mtx_GSEA))
# 
# count_filename <- paste(comparison,"count.csv", sep = "_")
# write.csv(counts_mtx_GSEA, count_filename)
# 
# # Retrieve annotation table for GSEA
# annotation_GSEA <- data.frame(Sample = rownames(filtered_data), Group = filtered_data$Phenotype)
# 
# # Get rid of ".dcc" so NIDAP can recognize sample names
# annotation_GSEA$Sample <- gsub("\\..*","",annotation_GSEA$Sample)
# 
# annot_filename <- paste(comparison,"annot.csv", sep = "_")
# write.csv(annotation_GSEA, annot_filename)

## Run DEG and generate volcano-------------------------------------------------

# Remove epithelial associated genes from Tcell population
if (grepl("Tcells",comparison)){
  starget_demoData <- starget_demoData[!rownames(starget_demoData) %in% epithelial_genes_to_rm,]
}

factors_to_recognize <- unlist(strsplit(comparison, "\\|"))

pData(starget_demoData)$testRegion <- 
  factor(pData(starget_demoData)$SampleCellType, c(factors_to_recognize[1], factors_to_recognize[2]))

pData(starget_demoData)[["slide"]] <- 
     factor(pData(starget_demoData)[["slide name"]])

table(pData(starget_demoData)$Case, pData(starget_demoData)$`slide name`)

results <- c()

mixedOutmc <-
    mixedModelDE(starget_demoData,
                 elt = "log_q",
                 # ~ allows you to specify function without calling object
                 modelFormula = ~ testRegion + (1 | Case),
                 groupVar = "testRegion",
  ...
  r_test <- r_test[, c("Gene", "Estimate", "Pr(>|t|)", "FDR")]
  ...
  #rev_results <- results
  #plot(rev_results$`Pr(>|t|)`,orig_results$`Pr(>|t|)`)

(l <- sapply(pData(starget_demoData), function(x) is.factor(x)))
write.csv(results, paste(comparison,"DEG_12_10_21_rev.csv", sep = "_"))
...
top_g <- c(top_g,
           results['Gene'][
               order(results['invert_P'], decreasing = TRUE)[1:25],],
             results['Gene'][
               order(results['invert_P'], decreasing = FALSE)[1:25],])
...
g <- ggplot(results,
  ...
  geom_point() +
  labs(x = paste("Enriched in", factors_to_recognize[2], "<- log2(FC) ->", "Enriched in", factors_to_recognize[1], sep = " "),
  ...
  geom_text_repel(data = subset(results, Gene %in% top_g & FDR < 0.05),
                  size = 7, point.padding = 0.15, color = "black",
                  min.segment.length = .1, box.padding = .2, lwd = 2) +
  theme_bw(base_size = 14) +
  theme(legend.position = "bottom", legend.text = element_text(size = 14), axis.title.x = element_text(size = 14),
        axis.title.y = element_text(size = 14), axis.text.x = element_text(size = 14), axis.text.y = element_text(size = 14))

ggsave(filename = paste(comparison,"stringent_QC_settings_volcano_12_10_21_rev.png",sep = "_"), plot = g, width = 9)
}


## Epithelial Clustering DEG and Volcano
# Prepare data for epithelial subset
target_demoData_for_epi_DEG <- target_demoData_for_DEG[,colnames(target_demoData_for_DEG) %in% colnames(target_demoData_Epithelial)] 

pData(target_demoData_for_epi_DEG)$kmeans_cluster <- pData(target_demoData_Epithelial)$kmeans_cluster[match(rownames(pData(target_demoData_for_epi_DEG)), rownames(pData(target_demoData_Epithelial)))]

# numeric kmeans
km_unique_clusters <- as.numeric(unique(sort(km_clusters)))

combn_column = 0

# Find all possible combination pairs of kmeans clusters
all_possible_combinations <- combn(km_unique_clusters, m = 2)

# GSEA counts and annotation
for (combn_column in 1:ncol(all_possible_combinations)){
  
  ## use rev for 2 vs 4, 3 vs 4, or 4 vs 5 since 4 is a tumor population
  if (all(all_possible_combinations[,combn_column] == c(2,4)) | all(all_possible_combinations[,combn_column] == c(3,4))){
  combn_pair <- rev(all_possible_combinations[,combn_column])
  }else{
  combn_pair <- all_possible_combinations[,combn_column]}
  
  ### For running in reverse order:
  #combn_pair <- rev(combn_pair)
 
  filtered_data <- pData(target_demoData_for_epi_DEG)[grepl(paste(combn_pair,collapse = "|"), pData(target_demoData_for_epi_DEG)$kmeans_cluster),]

# Check cells are present
print(unique(filtered_data$kmeans_cluster))

fsamplenames <- rownames(filtered_data)

starget_demoData_epi_clu <- target_demoData_for_epi_DEG[,fsamplenames]
...
# colnames(counts_mtx_GSEA) <- gsub("\\..*","",colnames(counts_mtx_GSEA))
# 
# annot_pair_name <- paste(combn_pair,collapse = "_vs_")
# 
# count_filename <- paste(paste("Cluster", annot_pair_name, sep = "_"),"count.csv", sep = "_")
# write.csv(counts_mtx_GSEA, count_filename)
# 
# # Retrieve annotation table for GSEA
# annotation_GSEA <- data.frame(Sample = rownames(filtered_data), Group = paste("Cluster",filtered_data$kmeans_cluster, sep ="_"))
# 
# annotation_GSEA$Sample <- gsub("\\..*","",annotation_GSEA$Sample)
# 
# annot_filename <- paste(paste("Cluster", annot_pair_name, sep = "_"),"annot.csv", sep = "_")
# write.csv(annotation_GSEA, annot_filename)

# Start DEG and Volcano for epithelial subcluster ------------------------------
pData(starget_demoData_epi_clu)$testRegion <- 
  factor(pData(starget_demoData_epi_clu)$kmeans_cluster, c(combn_pair[1], combn_pair[2]))

results <- c()

mixedOutmc <-
    mixedModelDE(starget_demoData_epi_clu,
                 elt = "log_q",
                 modelFormula = ~ testRegion + (1 | Case),
                 groupVar = "testRegion",
                 nCores = parallel::detectCores(),
  ...
  #results_epi_rev <- results
  
  #plot(results_epi$`Pr(>|t|)`,results_epi_rev$`Pr(>|t|)`)

(l <- sapply(pData(starget_demoData), function(x) is.factor(x)))

Epithelial_DEG_name <- paste(combn_pair, collapse = "_")
write.csv(results, paste("epithelial_km_clusters_test",Epithelial_DEG_name,"DEG_12_10_21.csv", sep = "_"))
...
g <- ggplot(results,
  ...
  geom_hline(yintercept = -log10(0.05), lty = "dashed") +
  geom_point() +
  labs(x = paste("Enriched in", paste("kmeans_cluster",combn_pair[2], sep = "_"), "<- log2(FC) ->", "Enriched in", paste("kmeans_cluster",combn_pair[1], sep = "_"), sep = " "),
       y = "Significance, -log10(P)",
       color = "Significance") +
  scale_color_manual(values = c(`FDR < 0.001` = "dodgerblue",
                                `FDR < 0.05` = "lightblue",
...
file_name <- paste(combn_pair, collapse = "_vs_")

ggsave(filename = paste(file_name,"epithelial_km_clusters_volcano_12_10_21.pdf",sep = "_"), plot = g, width = 9)
}
...
# setwd("/Users/bianjh/Desktop/Lydia_DSP")
# write.csv(results, paste(comparison,"DEG.csv", sep = "_"))
```
...
# below is work-around text to quick load DE results from a saved csv for convenience
# THIS SECTION WILL NOT BE SHOWN, is only for speed of iteration
#results2 <- as.data.frame(read.csv("~/DE_results_for_Tcells.csv", row.names = 1))
...
# results2$class <- pData(target_demoData)$Phenotype[match(results2$Subset,pData(target_demoData)[,1])]
#   
...
#   geom_hline(yintercept = -log10(0.05), lty = "dashed") +
#   geom_point() +
#   labs(x = paste("Enriched in", factors_to_recognize[1], "<- log2(FC) ->", "Enriched in", factors_to_recognize[2], sep = " "),
#        y = "Significance, -log10(P)",
#        color = "Significance") +
#   scale_color_manual(values = c(`FDR < 0.001` = "dodgerblue",
#                                 `FDR < 0.05` = "lightblue",
